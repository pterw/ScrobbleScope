...
## ScrobbleScope scoping

This file is the generic Mermaid AI Skills pack. Three things about how it
applies here:
...
- **Tool names.** The `mermaid-diagram-*` LM tools below exist only for GitHub
  ...
  and so covers Rules 1 and 2 at once.
- **Rule 1 is not optional here.** A sequence-diagram message containing `;`
  parses as a statement terminator and fails; that defect shipped once
  already, escaped as `&#59;` rather than removed. Validate before writing.

Repository rules in `AGENTS.md` win over anything in this file, including its
ASCII-only requirement for Markdown -- this file has been converted to comply.
It is tool-generated, so a regeneration may reintroduce non-ASCII characters.

## Workflow

1. Determine the diagram type and generate Mermaid syntax.
2. Validate syntax: correct first-line keyword, arrow types, balanced brackets.
3. Write the diagram to a `.mmd` file in the project.
...
1. Always call `mermaid-diagram-validator` before showing any diagram. If it
   returns errors, correct the syntax and call `mermaid-diagram-validator`
   again before writing the file or calling `mermaid-diagram-preview`. Do not
   present the diagram until validation passes.
...
4. Use `@mermaid-chart` slash commands when the user provides source files
   (code, schema, Dockerfile) as input or requests a specialized diagram type
   (ER, C4, cloud, Docker); otherwise generate directly.
