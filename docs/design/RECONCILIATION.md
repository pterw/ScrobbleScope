...
| Header pills | Prototype scaffolding, do not build | Production navigation: Home, Heatmap, Results, Unmatched. At mobile widths the destinations use a directly visible two-by-two grid with no horizontal scrolling; the single theme control follows page content. | Owner decision, `624ebb9`, refined after owner mobile review |
