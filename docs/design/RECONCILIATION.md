...
| Wide index composition | `1.1fr 1fr`, form capped at 380px | Current source declares `3fr 5fr`; hero and form use `1.075 * max(1, min(viewportWidth / 1920, viewportHeight / 1080))`, capped at `2.15`, and the centred form remains capped at `23.75rem`. That formula does not render a proportional composition: it divides window height by the 1080px design viewport instead of by the composition's own 673px height, so browser chrome makes the height term win on every real window. | `docs/superpowers/plans/2026-09-01-batch21-index-scaling-and-review-remediation.md` is the acceptance source. The defect is engine-independent -- Chromium and Firefox measure the same width to within 0.1px -- so the acceptance condition is realistic window geometry, not Firefox. |
