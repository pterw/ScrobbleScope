# SESSION_CONTEXT.md -- Reference Snapshot

**What this file is:**
This is a reference snapshot of `.claude/SESSION_CONTEXT.md` as it existed at
the end of the Batch 12 / post-audit development sprint (2026-02-23). It is
kept here for readers who want to understand the format and role of that file.

**What `.claude/SESSION_CONTEXT.md` is:**
A machine-managed dashboard that a new LLM agent session reads at startup to
understand the current project state without re-parsing the full PLAYBOOK.
It contains the current test count, active batch status, module structure,
dependency graph, and architecture overview. The `<!-- DOCSYNC:STATUS-* -->`
block is automatically refreshed by `scripts/doc_state_sync.py` from
PLAYBOOK truth -- it is a derived file, not a source file.

**Why it is gitignored:**
It is ephemeral agent working memory. Its content changes every session and
committing it would produce noisy diffs of no engineering value. CI does not
require it; `doc_state_sync.py` handles its absence gracefully (see
`_read_lines_optional()` in the script and commit `05c7b19` on `main`).

**Where the live file lives:** `.claude/SESSION_CONTEXT.md` (gitignored).

For more context see `DEVELOPMENT.md`, the section "The Orchestration Architecture".

---

<!-- BEGIN REFERENCE SNAPSHOT (2026-02-23) -->
