...
def get_lastfm_limiter():
    ...
    if limiter is None:
        limiter = AsyncLimiter(20, 1)  # Reduced from 20 to 5 requests per second to be safer
...
def normalize_name(artist, album):
    ...
    a = unicodedata.normalize("NFKD", artist).encode("ascii", "ignore").decode().lower().strip()
    b = unicodedata.normalize("NFKD", album).encode("ascii", "ignore").decode().lower().strip()
    
    # Remove common words and punctuation for better matching
    common_words = ['the', 'a', 'an', 'deluxe', 'edition', 'remastered', 'version', 
                    'expanded', 'anniversary', 'special', 'bonus', 'tracks', 'ep']
...
def unmatched():
    with unmatched_lock:
        unmatched_data = dict(UNMATCHED)  # Create a copy to avoid potential race conditions
...
async def fetch_recent_tracks_page_async(session, username, from_ts, to_ts, page, retries=3):
    url = "http://ws.audioscrobbler.com/2.0/"
    params = {
            ...
            async with limiter:
                ...
                async with session.get(url, params=params) as resp:
                    if resp.status == 429:
                        ...
                        logging.warning(f"⚠️ Last.fm rate limit hit. Retry after {retry_after} seconds.")
                        ...
                        logging.warning(f"❌ Unexpected Last.fm status {resp.status} on page {page}: {body[:200]}")
                        break
                    try:
                        data = await resp.json()
                        ...
                        body = await resp.text()
                        logging.error(f"❌ Invalid JSON from Last.fm page {page}. Body starts with: {body[:200]}")
                        break
        except Exception as e:
...
async def fetch_pages_batch_async(session, username, from_ts, to_ts, pages):
    ...
    logging.info(f"Completed batch fetch for pages {min(pages)} to {max(pages)}, got {len(results)} results")
    return results
...
async def fetch_all_recent_tracks_async(username, from_ts, to_ts):
    async with aiohttp.ClientSession() as session:
        ...
        for start in range(2, total_pages+1, BATCH):
            ...
            batch = await fetch_pages_batch_async(session, username, from_ts, to_ts, range(start, end))
...
async def fetch_spotify_album_release_date(session, artist, album, token):
    ...
    for attempt in range(3):
            ...
            async with limiter:
                ...
                async with session.get("https://api.spotify.com/v1/search", params=params, headers=headers) as response:
                    ...
                    if response.status != 200:
                        ...
                        logging.warning(f"❌ Spotify unexpected status {response.status} for {album} by {artist}. Body: {body[:200]}")
                        break
                    ...
                    if not items:
                        ...
                        async with session.get("https://api.spotify.com/v1/search", params=relaxed_q, headers=headers) as fallback:
                            relaxed_data = await fallback.json()
                            relaxed_items = relaxed_data.get("albums", {}).get("items", [])
                            if not relaxed_items:
                                logging.info(f"🔎 No match found on Spotify for {album} by {artist}")
        ...
        # more error logging
            ...
            logging.error(f"Spotify exception on attempt {attempt+1} for {album} by {artist}: {e}")

        # exponential backoff
        backoff = 2 ** attempt
        logging.debug(f"🔁 Backing off for {backoff} seconds before retrying {album} by {artist}")
...
async def process_albums(filtered_albums, year, sort_mode, release_scope, decade=None, release_year=None):
    logging.info(f"Processing {len(filtered_albums)} albums with filters: year={year}, release_scope={release_scope}, decade={decade}, release_year={release_year}")
    ...
    def matches_release_criteria(release_date):
        if not release_date:
            return False
        release_year_str = release_date.split('-')[0] if '-' in release_date else release_date
        try:
    ...
    def get_user_friendly_reason(release_date, release_scope, decade=None, release_year=None):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for (artist, album), data in filtered_albums.items():
            album_details = await fetch_spotify_album_release_date(session, artist, album, token)
        ...
        if sort_mode == "playtime":
            ...
            if results:
                ...
                for album in results:
                    album["proportion_of_max"] = album["play_time_seconds"] / max_play_time * 100
                    album["proportion_of_total"] = album["play_time_seconds"] / total_play_time * 100
        else:
            ...
            if results:
                ...
                # Add proportion data
                for album in results:
                    album["proportion_of_max"] = album["play_count"] / max_play_count * 100
                    album["proportion_of_total"] = album["play_count"] / total_play_count * 100
