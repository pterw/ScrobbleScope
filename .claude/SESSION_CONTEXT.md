...
| Batch 21 status | **Active.** WP-0 through WP-5 are done. WP-7 (unmatched page + reason_code) is active (Commit 1 backend complete; Commit 2 UI rebuild in progress). WP-7 is next; Task 6 timing follows the canonical remediation plan linked in PLAYBOOK Section 3. WP-6 is absorbed into WP-3; WP-7 and WP-8 keep their numbers. Adobe Fonts kit `rwy8ghw` remains active. Definition: `BATCH21_DEFINITION.md`. See PLAYBOOK Sections 3-4 for the work order and history. |
